...
int main(int argc, char *argv[])
{
  ...
  string imageFilename = argv[1];           // 测试图像名
  string planFilename = argv[2];            // 模型名字
  string labelFilename = argv[3];           // 标签文件
  string inputName = argv[4];               // 模型输入 tensor 的名字
  string outputName = argv[5];              // 模型输出 tensor 的名字
  string preprocessingFn = argv[6];         // 预处理的方式
  ...
  initLibNvInferPlugins(&gLogger, "");            // 注册插件，那python中应该有这样一句来初始化
