...
using nvinfer1::plugin::ONNXScatterNDDynamic;
using nvinfer1::plugin::ONNXScatterNDDynamicCreator;
...
ONNXScatterNDDynamic::ONNXScatterNDDynamic(const std::string &name)         // 初始化
    : mLayerName(name) {}
...
nvinfer1::DimsExprs ONNXScatterNDDynamic::getOutputDimensions(      // 获得输出的维度
...
int ONNXScatterNDDynamic::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,      // @@插件的前向计算函数
                                  const nvinfer1::PluginTensorDesc *outputDesc,
                                  const void *const *inputs,
                                  void *const *outputs, void *workSpace,
                                  cudaStream_t stream) {
  const int *dims = &(inputDesc[0].dims.d[0]);          // data 维度指针
  const int *indices_dims = &(inputDesc[1].dims.d[0]);  // indics 维度指针
  int nbDims = inputDesc[0].dims.nbDims;                // data 维度数
  int indice_nbDims = inputDesc[1].dims.nbDims;

  const void *data = inputs[0];         // 每个具体代表什么，从哪里看？？
  const void *indices = inputs[1];      // 从 onnx 的官方查询：https://github.com/onnx/onnx/blob/master/docs/Changelog.md#ScatterND-11
...
int ONNXScatterNDDynamic::getNbOutputs() const { return 1; }        // 有几个变量输出
